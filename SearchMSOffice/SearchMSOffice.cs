...
namespace SearchMSOffice
{
   /// <summary>
   /// Searching in MSOffice files like Word, Excel and Powerpoint
   /// </summary>
   public class SearchMSOffice : ISearchInFolderPlugIn
   {
      ...
      public String Name { get; } = "SearchMSOffice";
      public String Description { get; } = "Searching in MSOffice files like Word, Excel and Powerpoint";
      public Guid ID { get; } = new Guid("926d245e-ad5f-4a3a-84ca-5ce68152b07b");
      ...
      public SearchMSOffice()
      {
         string resourceName = "SearchMSOffice.defintions.json";
      ...
      /// <summary>
      /// put all the extension into one list
      /// </summary>
      /// <returns></returns>
      ...
      private List<FoundItem> SearchInWordDocument(string sFilePath, string sSearchTerm, bool bMatchCaseIn, bool bWholeWordIn)
      {
          ...
          using (WordprocessingDocument wordDoc = WordprocessingDocument.Open(sFilePath, false))
          {
              ...
              if (body != null)
              {
                  ...
                  {
                      // Process paragraphs
                      if (element is Paragraph paraObj)
                      {
                          paragraphNumber++; // Count paragraphs
                          
                          string sParaText = paraObj.InnerText;
                          
                          // Count line breaks within the paragraph
                          var lineBreaks = paraObj.Descendants<DocumentFormat.OpenXml.Wordprocessing.Break>()
                              .Where(br => br.Type == null || br.Type.Value == BreakValues.TextWrapping);
                          
                          int linesInParagraph = 1 + lineBreaks.Count(); // At least 1 line per paragraph
                          totalLineNumber += linesInParagraph;
